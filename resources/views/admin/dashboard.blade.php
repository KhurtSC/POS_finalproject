<x-layout title="Admin Dashboard">
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-2">
        <section class="rounded-lg border border-slate-200 bg-white p-5 text-slate-900 shadow-sm">
    ...
    <section class="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-900 shadow-sm">
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse ($recentSales as $sale)
                        <tr>
                            <td class="px-5 py-4 font-bold text-slate-950">{{ $sale->reference }}</td>
                            <td class="px-5 py-4">{{ $sale->cashier->name ?? '—' }}</td>
                            <td class="px-5 py-4">{{ $sale->items_count }}</td>
                            <td class="px-5 py-4 font-semibold text-slate-950">₱{{ number_format($sale->total_amount, 2) }}</td>
                            <td class="px-5 py-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center font-semibold text-slate-500">No sales recorded yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
